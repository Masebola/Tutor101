...
create policy "tutor manages own sessions" on sessions for all using (tutor_id = auth.uid());
...
create policy "tutor manages own resources" on resources for all using (uploaded_by = auth.uid());
...
create policy "tutor posts announcements" on announcements for insert with check (posted_by = auth.uid());

-- support_requests: the student owns theirs; the module's approved tutor (or an admin) can read/answer.
create policy "student manages own requests" on support_requests for all using (student_id = auth.uid());
...
-- reviews: a student can leave one for their own session; anyone can read ratings.
create policy "student can leave a review" on reviews for insert with check (student_id = auth.uid());
